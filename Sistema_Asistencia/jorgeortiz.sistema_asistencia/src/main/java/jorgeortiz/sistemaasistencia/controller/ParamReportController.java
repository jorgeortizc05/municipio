...
public class ParamReportController implements Serializable {
	...
	public void getRestServidor1(String cedula) {
		employee = justDAO.getEmpleadosServidor(cedula);
		this.getRestBJustificaciones1(cedula);
	}

	public void getRestBJustificaciones1(String cedula) {
	...
	public String checkNombresYapellidosServidor(Integer cod_servidor) {
		...
		for (SERVIDOR s : this.getEmployee()) {
			...
			if (s.getCODIGO().equals(cod_servidor)) {
				String n = s.getAPELLIDO_PATERNO() + " " + s.getAPELLIDO_MATERNO() + " " + s.getPRIMER_NOMBRE() + " "
						+ s.getSEGUNDO_NOMBRE();
				return n;
			}
		}
	...
	public void checkBiometrico(ActionEvent actionEvent) throws ParseException {
		...
		try {
			...
			if (!df.format(this.getFecha1()).equals(null) || !df.format(this.getFecha2()).equals(null)
					|| !this.getBiometrico().isEmpty()) {
				...
				if (band == true) {
					if (this.getTiempo1().isEmpty() || this.getTiempo1().isEmpty()) {
						...
						this.setNombre_pdf(this.checkRazonSocialEmpApellido(this.getBiometrico()) + " "
								+ this.checkRazonSocialEmpNombre(this.getBiometrico()));
						...
						if (!cedula.equals(null)) {
							this.getRestServidor1(cedula);
						} else
							System.out.println("SIn cedulaaaa");
					} else if (!this.getTiempo1().isEmpty() && !this.getTiempo2().isEmpty()) {
						...
						addMessage("Tiempos mal inttroducidos");
				} else
					addMessage("Código de biométrico no encontrado");
			} else
				addMessage("Datos mal introducidos!");
			...
			if (!this.getFecha1().equals(null) || !this.getFecha2().equals("")) {
				if (!this.getFiltro_dep().equals(null)) {
					if (this.getTiempo1().isEmpty() || this.getTiempo1().isEmpty()) {
						...
						addMessage("Tiempos mal inttroducido");
				} else
					addMessage("Datos mal ingresado");
			} else
			...
			if (!this.getFecha1().equals(null) || !this.getFecha2().equals("")) {
				if (!this.getFiltro_dep().equals(null)) {
					if (this.getTiempo1().isEmpty() || this.getTiempo1().isEmpty()) {
						...
						this.setNombre_pdf(this.checkRazonSocialEmpApellido(this.getFiltro_dep() + "") + " "
								+ this.checkRazonSocialEmpNombre(this.getFiltro_dep() + ""));
						...
						addMessage("Tiempos mal introducido");
				} else
					addMessage("Datos mal ingresado");
			} else
				addMessage("Datos mal introducido!");
...
//		}else return a;
	...
	public void preProcessPDF(Object document) throws IOException, BadElementException, DocumentException {
		...
		pdf.add(new Paragraph(this.getNombre_pdf(),
	...
	public MainReporteDAO getMdao() {
		return mdao;
	}

	public void setMdao(MainReporteDAO mdao) {
		this.mdao = mdao;
	}
	...
	public MainJustDAO getJustDAO() {
		return justDAO;
	}

	public void setJustDAO(MainJustDAO justDAO) {
		this.justDAO = justDAO;
	}
