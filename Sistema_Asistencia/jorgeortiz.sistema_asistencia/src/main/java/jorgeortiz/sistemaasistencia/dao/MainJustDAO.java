...
//		}
	...
	public List<ACC_PER> getAccPer(String cedula,Date fecha1, Date fecha2) throws NullPointerException{
		Query query=emN.createQuery("select a from ACC_PER a, SERVIDOR b where b.CEDULA = :cedula and a.SER_CODIGO = b.CODIGO and a.ACCION IN ("+"'LSM','LSP','VAC','LSL','LSS','Z','DCS','CCR','CSR'"+") AND a.FECHA_VIGENCIA > :fecha1 and a.FECHA_VIGENCIA < :fecha2",ACC_PER.class);
		query.setParameter("cedula", cedula);
		query.setParameter("fecha1", fecha1);
		query.setParameter("fecha2", fecha2);
