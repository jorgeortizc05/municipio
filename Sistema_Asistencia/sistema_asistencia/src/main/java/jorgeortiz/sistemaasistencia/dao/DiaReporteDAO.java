...
public class DiaReporteDAO implements Serializable {
	...
	public List<TIMBRE> getTimbresEmpleado(Date fecha1, Date fecha2, String clave) throws NullPointerException{
		try{
			Query query=emF.createQuery("select r from TIMBRE r where r.FECHA_HORA_TIMBRE > :fecha1 and r.FECHA_HORA_TIMBRE< :fecha2 and r.CODIGO_EMPLEADO = :clave order by r.FECHA_HORA_TIMBRE",TIMBRE.class);
			query.setParameter("fecha1", fecha1);
			query.setParameter("fecha2", fecha2);
...
//	}
	public String getRazonSocialEmpApellido(String cod_bio) throws NullPointerException{
		...
		try{
		Query query=emF.createQuery("select r FROM EMPLEADO r WHERE r.CODIGO_EMPLEADO = :cod_bio",EMPLEADO.class);
	...
	public String getDepartamento(String cod_bio) throws NullPointerException{
		//	System.out.println("Leyendo Empleado con id "+cod_bio);
			...
			Query queryDep=emF.createQuery("select d FROM DEPARTAMENTO d where d.DEPA_ID = "+emp.getDEPA_ID(),DEPARTAMENTO.class);
