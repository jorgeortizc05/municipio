...
public class MainJustDAO implements Serializable {
	...
	public List<BJUSTIF> getBjust() throws NullPointerException{
		Query query=emN.createQuery("select r from BJUSTIF r",BJUSTIF.class);
...
//		System.out.println("fecha2> "+fecha2+" ---- "+fecha2.toString()+" ----- "+fecha2.getTime());
		Query query=emN.createQuery("select r from BJUSTIF r, SERVIDOR s where s.CEDULA = :cedula and r.FECHA > :fecha1 and r.FECHA < :fecha2 and r.SER_CODIGO = s.CODIGO order by r.FECHA DESC",BJUSTIF.class);
		query.setParameter("cedula", cedula);
...
//		}
	...
	public List<ACC_PER> getAccPer(String cedula,Date fecha1, Date fecha2) throws NullPointerException{
		Query query=emN.createQuery("select a from ACC_PER a, SERVIDOR b where b.CEDULA = :cedula and a.SER_CODIGO = b.CODIGO and a.ACCION IN ("+"'LSM','TC1','TN1','TA6','TNR','TAC','PCF','TSE','CNP','FIN','RNL','LAL','ACR','VBU','A_R','CAP','LSP','VAC','LSL','LSS','Z','DCS','CCR','CSR'"+") AND a.FECHA_VIGENCIA > :fecha1 and a.FECHA_VIGENCIA < :fecha2",ACC_PER.class);
		query.setParameter("cedula", cedula);
		query.setParameter("fecha1", fecha1);
		query.setParameter("fecha2", fecha2);
