...
//	        String dateInString = "03/03/2017";
	        ...
	        try {
				...
				String fecha_dia2=fecha_dia+" 23.00.00 a";
...
//	            this.setTimbres(dao.getTimbres(date));
	            this.setTimbres(dao.getTimbres(date, date1));
...
//	            	}
		            Mensajes.addMessage(this.getTimbres().size()+" Registros en este día!!");
	            }else Mensajes.addMessage(" No hay registros");
	            	if(this.getTimbres().equals(null))Mensajes.addMessage(" Nada que actualizar!!");
...
//	            		System.out.println("fecha_tim > "+t.getFECHA_HORA_TIMBRE());
//	            	}
		            Mensajes.addMessage(this.getTimbresTemp().size()+" Registros en este día temp!!");
	            }else Mensajes.addMessage(" No hay registros temp");
		} catch (ParseException e) {
...
//	            		System.out.println("fecha_tim > "+t.getFECHA_HORA_TIMBRE());
//	            	}
		            Mensajes.addMessage(this.getTimbresAtrasado().size()+" Registros en este día temp!!");
	            }else Mensajes.addMessage(" No hay registros temp");
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
...
//	            	System.out.println("total no timbran "+cont);
		            Mensajes.addMessage(this.getEmps_sin_timbres().size()+" Registros en este día temp!!");
	            }else Mensajes.addMessage(" No hay registros temp");
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
